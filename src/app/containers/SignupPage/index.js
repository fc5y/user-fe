...
function SignupPage({ history }) {
  ...
  const [popupState, setPopupState] = React.useState(POPUP_STATE.NONE);
  ...
  const submit = React.useCallback(async () => {
    ...
    if (hasBlockingError(getErrors(sanitizedData))) {
      setPopupState(POPUP_STATE.ERROR);
      return;
    }
    ...
    if (!!error || !response || !response.username) {
      ...
      setPopupState(POPUP_STATE.SUCCESS);
    }
  }, [data]);

  const handleClosePopup = React.useCallback(() => {
    setPopupState(POPUP_STATE.NONE);
  });
  ...
  return (
    <MainPanel.Container>
      {redirectState === true ? <Redirect to="/login" /> : ''}
      {popupState === POPUP_STATE.NONE ? (
        ''
      ) : popupState === POPUP_STATE.ERROR ? (
        <Popup
          onClose={handleClosePopup}
          title="Đăng ký thất bại"
          content="Đã xảy ra lỗi trong quá trình đăng ký. Vui lòng kiểm tra lại."
          buttonText="OK"
          variant="error"
        />
      ) : (
        <Popup
          ...
          content="Cảm ơn bạn đã đăng ký tham gia FYT Code Cup! Vui lòng đăng nhập để tiếp tục."
